...
args = {
    ...
    'fixed_matrix': False, #fixes a single matrix across entire alphazero algorithm. If set to True, then self play games generated in each iteration have different sensing matrices. 
    ...
    'numEps': 10, #dictates how many games are played each iteration of the algorithm
    'maxlenOfQueue':10000, #dictates total number of game states saved(not games). 
    'numItersForTrainExamplesHistory': 5, #controls the size of trainExamplesHistory, which is a list of different iterationTrainExamples deques. 
    ...
    'numMCTSSims': 50, #For each move, numMCTSSims is equal to the number of MCTS simulations in finding the next move during self play. 
