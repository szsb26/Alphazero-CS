...
class batch_MCTS():
    ...
    def search_traversetoLeaf(self, MCTS_object, State):
        ...
        if s not in MCTS_object.Ps:
            ...
            State.compute_x_S(self.args, MCTS_object.game_args)
    ...
    #3)Initialize visit count to 0 in MCTS_object.Ns[leaf]
        ...
        valids = MCTS_object.game.getValidMoves(MCTS_object.game_args, leaf_state) #returns a numpy vector of 0 and 1's which indicate valid moves from the set of all actions
