...
class CSGame(): 
    ...
    def getValidMoves(self, game_args, state): #O(n) operation
    ...
    #the stopping action can be taken. n is number of columns of sensing matrix A. 
        ...
        if state.col_indices:
            S = state.col_indices #note that when we compute the termreward for initial state, THIS WILL RETURN AN ERROR because col_indices of initial state is []   
            A_S = game_args.sensing_matrix[:,S]
            x_S = np.matmul(state.inverse, state.ATy)
            ...
            res_norm_squared = np.linalg.norm(game_args.obs_vector)**2

        if res_norm_squared > 1e-5:
    ...
    def keyRepresentation(self, state): #convert state.action_indices into a string for hashing in MCTS        
        hashkey_tuples = tuple(state.action_indices.tolist())
