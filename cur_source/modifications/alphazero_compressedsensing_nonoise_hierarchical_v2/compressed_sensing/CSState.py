...
class State():
    ...
    def compute_x_S(self, args, Game_args): #compute feature vectors for feeding into NN. Labels are returned by computeTermReward. 
        if self.col_indices: #If self.col_indices is not an empty list(meaning that we are not at the start state in which we have not chosen any columns)
            ...
            #FEATURE 2:
            self.feature_dic['y'] = Game_args.obs_vector
