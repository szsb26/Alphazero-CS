...
class NNetWrapper(): 
    def __init__(self, args): 
        ...
        self.nnet = NetArch2(args) #UNCOMMENTING THIS LINE CAUSES THREADLOCK ERROR "can't pickle _thread.lock objects"
    ...
    #OUTPUT: two matrices (p_as, v)
        ...
        for MCTS_object, States_list in MCTS_States_list:
            ...
            if MCTS_object.Es[last_state.keyRep] == 0:
                ...
                y_feature = MCTS_object.features_s[last_state.keyRep]['y'] #a numpy array of dim (1, col. size of A)
                
                #append features of MCTS_object into a list for batch prediction
                x_l2_temp.append(x_l2_feature)
                y_temp.append(y_feature)
        ...
        p_as_matrix, v_matrix = self.nnet.model.predict([x_l2_temp, y_temp])
