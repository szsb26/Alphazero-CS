...
class MCTS():
    ...
    def __init__(self, game, nnet, args, Game_args, identifier = None):
        ...
        self.nnet = nnet    #nnet is a NNetWrapper object #This self variable is not needed if no search is going to be done below
    ...
    def getActionProb(self, canonicalBoard, temp=1): #temp = 1, the default option, has getActionProb return a probability distribution. The next action is chosen at random from this distribution. 
        ...
        if temp==0:
            ...
            return probs #here probs is a list of 0 with a single 1!!!
    ...
    def search(self, canonicalBoard): 
        ...
        #4)self.Ns[s]
        if s not in self.Ps: 
            canonicalBoard.compute_x_S(self.args, self.game_args)
            ...
            valids = self.game.getValidMoves(self.game_args,canonicalBoard) #returns a numpy vector of 0 and 1's which indicate valid moves from the set of all actions
