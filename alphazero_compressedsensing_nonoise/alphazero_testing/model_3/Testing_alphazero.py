...
args = {
    ...
    'numMCTSSims': 1000, #For each move, numMCTSSims is equal to the number of MCTS simulations in finding the next move during self play. 
    ...
    'alpha': 1e-3, #note that reward for a terminal state is -alpha||x||_0 - gamma*||A_S*x-y||_2^2. The smaller alpha is, the more weight the algorithm gives in selecting a sparse solution. 
...
for s in range(1, args['sparsity']):
    ...
    with open(obsy_filepath) as obsy, open(sparsex_filepath) as sparsex:
        ...
        for y, x in zip(reader_y, reader_x):
            ...
            #print('OMP prediction is: ' + str(predicted_OMPx))
            predicted_l1x = Algorithms.l1(A, y)
            #print('l1 prediction is: ' + str(predicted_l1x))
            
            error_alphazero = np.linalg.norm(Alphazero.game_args.sparse_vector - predicted_x)**2
            error_OMP = np.linalg.norm(x - predicted_OMPx)**2
            #print('OMP_error is: ' + str(error_OMP))
            error_l1 = np.linalg.norm(x - predicted_l1x)**2
            #print('l1_error is: ' + str(error_l1))
...
print('Recovery of l1-minimization is: ' )
print(str(l1_accuracy))
print('----------------------------------------------------')
print('')
